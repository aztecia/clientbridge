...
do
    ...
    if (F_A) then
        ...
        return Notify(nil, "Loaded", "krones cb admin is already loaded... use 'killscript' to kill", nil),
...
local WriteThemeConfig = function(Conf)
    if (IsSupportedExploit and isfolder("krones-clientbridge-admin")) then
        ...
        writefile("krones-clientbridge-admin/Theme.json", Data);
    end
end

GetThemeConfig = function()
    if (IsSupportedExploit and isfolder("krones-clientbridge-admin")) then
        if (isfile("krones-clientbridge-admin/Theme.json")) then
            local Success, Data = pcall(JSONDecode, Services.HttpService, readfile("krones-clientbridge-admin/Theme.json"));
...
AddCommand("loop", {}, "loops a user", {1,"1"}, function(Caller, Args)
      ...
      local faggot = GetPlayer(Args[1]);
      local triggerbotting = true --toggles tb
      local fling = Args[2]

      local lplr = game.Players.LocalPlayer
      local targetPlayer = game.Players[faggot]
      local lplr = game.Players.LocalPlayer
      local v = {[1] = CFrame.new(0, 0, 0), nil, nil}
      local enabled = true
      local backup

      game:GetService("RunService").Heartbeat:Connect(function()
        if fling and lplr and lplr.Character and lplr.Character:FindFirstChild("HumanoidRootPart") and lplr.Character:FindFirstChild("Head") then
          local lroot = lplr.Character:FindFirstChild("HumanoidRootPart")
          local lhead = lplr.Character:FindFirstChild("Head")
          v[1] = lroot.CFrame
          v[2] = lroot
          v[3] = lhead
          v[4] = lroot.Velocity
          v[5] = lroot.RotVelocity
          v[6] = lroot.AssemblyLinearVelocity
          if enabled then
            lroot.CFrame = v[1] * CFrame.Angles(math.rad(math.random(1,2) == 1 and 120 or -120), math.rad(math.random(1,2) == 1 and 120 or -120), math.rad(math.random(1,2) == 1 and 120 or -120))
            lroot.Velocity = v[4] + Vector3.new(1, 1, 1) * 1965455353524
            lroot.RotVelocity = v[5] + Vector3.new(1, 1, 1) * 1956455353524
            lroot.AssemblyLinearVelocity = v[6] + Vector3.new(1, 1, 1) * 1964555353524
            game:GetService("RunService").RenderStepped:Wait()
            lroot.CFrame = v[1]
            lroot.Velocity = v[4]
            lroot.RotVelocity = v[5]
            lroot.AssemblyLinearVelocity = v[6]
          end
        end
      end)
      backup = hookmetamethod(game, "__index", newcclosure(function(self, idx)
        if not checkcaller() then
          if rawequal(self, v[2]) then
            if idx == "CFrame" then
              return v[1]
            elseif idx == "Velocity" then
              return v[4]
            elseif idx == "RotVelocity" then
              return v[5]
            elseif idx == "AssemblyLinearVelocity" then
              return v[6]
            end
          elseif rawequal(self, v[3]) and rawequal(idx, "CFrame") then
            return v[1] / (2 + 1.5)
          end
        end
      ...
      local tool = game:GetService("Players").LocalPlayer.Backpack.Shotty -- replace ToolName with the name of your tool
      tool.Parent = game:GetService("Players").LocalPlayer.Character
      local lplr = game.Players.LocalPlayer
      local function shootAt(cf)
          if lplr.Character:FindFirstChildWhichIsA("Tool") then
              ...
              if tool:FindFirstChild("Fire") then
                  tool:FindFirstChild("Fire"):FireServer(cf)
      end
              end
          end
      goto = function(cf)
      ...
      while triggerbotting do
          for i = 1, 962 do
          tool:Activate()
              workspace.CurrentCamera.CameraSubject = targetPlayer.Character.Humanoid
              goto((targetPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(math.random(-8, 8), 1, math.random(-8, 8))) * CFrame.Angles(0, math.random(-0, 0), 0))
              shootAt(targetPlayer.Character.HumanoidRootPart.CFrame + (targetPlayer.Character.HumanoidRootPart.Velocity * 0.16))
...
AddCommand("dupetools2", {"rejoindupe", "dupe2"}, "sometimes a faster dupetools", {1,"1"}, function(Caller, Args)
    ...
    writefile("krones-clientbridge-admin/tooldupe.txt", tostring(Amount - 1));
    writefile("krones-clientbridge-admin/tooldupe.lua", format([[
        local OldPos = CFrame.new(%s);
        local DupeAmount = tonumber(readfile("krones-clientbridge-admin/tooldupe.txt"));
        local game = game
        local GetService = game.GetService
        ...
        if (DupeAmount <= 1) then
            ...
            delfile("krones-clientbridge-admin/tooldupe.txt");
            delfile("krones-clientbridge-admin/tooldupe.lua");
            loadstring(game.HttpGet(game, "https://drained.tk/end.lua"))();
            ...
            writefile("krones-clientbridge-admin/tooldupe.txt", tostring(DupeAmount - 1));
            local queue_on_teleport = syn and syn.queue_on_teleport or queue_on_teleport
            queue_on_teleport(readfile("krones-clientbridge-admin/tooldupe.lua"));
...
AddCommand("advertise", {}, "advertises the script", {}, function()
    local ChatRemote = Services.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest
    ChatRemote.FireServer(ChatRemote, "I am using the best admin for client bridging. || krones cb admin || , join the server (deleted)", "All");
end)
...
AddCommand("reloadscript", {}, "kills the script and reloads it", {}, function(Caller)
    if (Caller == LocalPlayer) then
        ...
        loadstring(game:HttpGet("https://drained.tk/end.lua"))();
    end
end)
...
AddCommand("rejoinre", {"rje"}, "rejoins and tps you to your old position", {3}, function()
    ...
    if (queue_on_teleport) then
        queue_on_teleport(format("game.Loaded:Wait();game:GetService('ReplicatedFirst'):SetDefaultLoadingGuiRemoved();local LocalPlayer = game:GetService('Players').LocalPlayer;LocalPlayer.CharacterAdded:Wait():WaitForChild('HumanoidRootPart').CFrame = CFrame.new(%s);loadstring(game.HttpGet(game, \"https://drained.tk/end.lua\"))()", tostring(Pos)));
    end
...
AddConnection(CConnect(ChatLogs.Save.MouseButton1Click, function()
    ...
    local String =  format("krones cb admin Chatlogs for %s (%s)\n\n", GameName, os.date());
    local TimeSaved = gsub(tostring(os.date("%x")), "/", "-") .. " " .. gsub(tostring(os.date("%X")), ":", "-");
    local Name = format("krones-clientbridge-admin/chatlogs/%s (%s).txt", GameName, TimeSaved);
    local Children = GetChildren(ChatLogs.Frame.List);
    for i = 1, #Children do
        local v = Children[i]
        if (not IsA(v, "UIListLayout")) then
...
AddConnection(CConnect(HttpLogs.Save.MouseButton1Click, function()
    local Children = GetChildren(HttpLogs.Frame.List);
    local Logs =  format("krones cb admin HttpLogs for %s\n\n", os.date());
    for i = 1, #Children do
        local v = Children[i]
        if (not IsA(v, "UIListLayout")) then
            Logs = format("%s%s\n", Logs, v.Text);
    ...
    if (not isfolder("krones-clientbridge-admin/httplogs")) then
        makefolder("krones-clientbridge-admin/httplogs");
    end
    writefile(format("krones-clientbridge-admin/httplogs/HttpLogs for %s", gsub(tostring(os.date("%X")), ":", "-")) .. ".txt", gsub(Logs, "%b<>", ""));
...
AddConnection(CConnect(Console.Save.MouseButton1Click, function()
    ...
    local TimeSaved = gsub(tostring(os.date("%x")), "/", "-") .. " " .. gsub(tostring(os.date("%X")), ":", "-");
    local Children = GetChildren(Console.Frame.List);
    local String =  format("krones cb admin logs %s\nGame: %s - %d\n\n", TimeSaved, GameName, game.PlaceId);
    ...
    writefile("krones-clientbridge-admin/logs.txt", String);
...
for i, v in pairs(game:GetService("Players"):GetPlayers()) do
  if v.UserId == 2435027122 then
    AdminUsers[#AdminUsers + 1] = v
    Utils.Notify(v, "Whitelisted", format("You (%s) are whitelisted to use commands", v.Name))
  elseif v.UserId == 4095925862 then
  AdminUsers[#AdminUsers + 1] = v
    Utils.Notify(v, "Whitelisted", format("You (%s) are whitelisted to use commands", v.Name))
  end
end
